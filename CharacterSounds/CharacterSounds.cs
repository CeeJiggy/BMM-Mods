...
namespace CharacterSounds
{
    internal class CharacterSounds : MonoBehaviour
    {
        ...
        public void Update()
        {
            if (MainGame.s_Instance != null)
            {
                Chara.kHasVoiceCharaTop = Chara.eKind.MaxCount;
                if (MainGame.s_Instance.m_CharaSelectDatum.m_CharaKind == Chara.eKind.Kiryu)
                {
                    MainGame.s_Instance.m_SoundCharaCueSheet.m_CueSheet = sound_id.cuesheet.vo_aiai;
                    MainGame.s_Instance.cueSheetLoad();
                }
            }
        }
    }
